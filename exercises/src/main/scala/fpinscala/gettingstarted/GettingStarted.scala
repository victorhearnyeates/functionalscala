...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def loop(currentIteration: Int, currentNumber: Int, previousNumber: Int): Int =
      if (currentIteration >= n) currentNumber
      else loop(currentIteration + 1, currentNumber + previousNumber, currentNumber)
